...
string PersMessage::getCharValue(int fieldNumber) const // геттер для вывода полей класса типа string
{
	switch (fieldNumber)
	{
...
void PersMessage::setValue(int numValue, string value) // сеттер для инициализации полей типа string
{
...
void Chat::showMess(string curName)              // показать сообщения
{
	...
	cout << "               ***               " << endl;  // оформление сообщений для лучшего восприятия информации
